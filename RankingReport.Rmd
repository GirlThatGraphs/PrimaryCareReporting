...
#
MM_Green_Import <- read_excel("Data/MM scorecard 2022-23 September final.xlsx",
                       sheet = "Ranked scorecard 2022-23",
                       range = "A1:AX65")

MM_Green_Hist <- read_excel("Data/MM scorecard 2022-23 August final.xlsx",
                       sheet = "Ranked scorecard 2022-23",
                       range = "A1:AX65") %>% 
...
MM_Diabeties <- MM_Green_Import %>%
    filter(`...2` != 'L85612') %>% 
    select(...2,
           `11. Patients with all 8 diabetes care processes undertaken in the last 12 months`) |> 
  mutate(`11. Patients with all 8 diabetes care processes undertaken in the last 12 months` = percent(`11. Patients with all 8 diabetes care processes undertaken in the last 12 months`))
```
...
GPSurvey <- read_excel("O:/BSS/BI/Intelligence Services/BSSW/Somerset/Reports/PrimaryCareReporting/Data/GPSurvey.xlsx") %>% 
...
Workforce_List_Date <- WorkforceAll |> 
  select(MONTH_DATE) |> 
  filter(month(MONTH_DATE) %in% c(3,6,9,12)) 

Workforce_List_Date <- max(Workforce_List_Date$MONTH_DATE)
#
...
WorkforceList <- WorkforceAll %>%
  ...
  mutate(List_Risk_RankC = case_when(Risk_List_Change == 0 ~ "low",
                                     ...
                                     TRUE ~ "error"),
         CHANGE_PATIENTS_ratio = percent(CHANGE_PATIENTS_ratio, accuracy = 0.1)) %>% 
  ungroup() %>% 
  arrange(MONTH_DATE) %>% 
  group_by(`MergePracticeCode`) %>% 
...
PCOGRating <- data.frame (Name  = c("West Coker Surgery",
                                    "Penn Hill Surgery", 
                          ...
                          Code = c("Y01163","L85017", "L85016", "L85010", "L85028", "L85061",   "L85012", "L85047", "L85019"),
                          RAG = c("Amber", "Amber", "Red", "Unrated", "Red", "Amber", "Amber", "Amber", "Amber"))
```
...
LKUP2%>% 
  ...
  datatable(colnames = c("Practice Code","Practice Name","PCN","Current Risk","Previous Risk",
                         ...
                         "GP Head Count", "GP Head Count Last Month", "Change in GP Head Count",
                         "GPs Aged 55+", "GPs Aged 55+ Last Month", "Change in GPs Aged 55+"),
            filter = 'top',
            rownames = FALSE,
            options = list(dom = 'ltp'))
```
...
LKUP2%>% 
  ...
  datatable(colnames = c("Practice Code","Practice Name","PCN","Current Risk","Previous Risk",
                         "Nurse Head Count", "Nurse Head Count Last Month", "Change in Nurse Head Count",
                         "Nurses Aged 55+", "Nurses Aged 55+ Last Month", "Change in Nurses Aged 55+"),
            filter = 'top',
            rownames = FALSE,
            options = list(dom = 'ltp'))
```
...
The table below ranks practices by prevalence of indicators considered high risk across all 11 risk indicators.
...
The table below ranks practices by prevalence of indicators considered high risk illustrating the current perceived risk in each of the 11 risk indicators.
...
LKUP2 %>% 
  ...
  datatable(colnames = c("Practice Code","Practice Name","PCN",
                         "GP Workforce", "Nurse Workforce", "Patient List","GP Survey", "Medicines Management Scorecard", "A and E Attendances", "Emergency Admissions", "Advice and Guidance", "Premises", "Core Contract Directed Enhanced Services", "Standard Contract Enhanced Services"),
            filter = 'top',
            rownames = FALSE,
            options = list(scrollX = TRUE, dom = 'ltp'),
            escape = FALSE) %>% 
    formatStyle(3:14, color = styleEqual(c('h', 'm', 'l'), c('red', 'orange', 'green')))
```
...
LKUP2 %>% 
  ...
  datatable(colnames = c("Practice Code","Practice Name","PCN","Practice Ranking","Risk", "Risk Last Quarter", "Partner Head Count", "Partner Head Count Last Month","Partner Head Count Change","GP (All)","GP Head Count (EXRL)","GP Head Count (EXRL) Last Month","GP Head Count (EXRL) Change","GP FTE (EXRL)", "GP FTE (EXRL) Last Month","GP FTE (EXRL) Change", "GPs aged 55+ (EXRL)", "GPs aged 55+ (EXRL) Last Month", "GPs aged 55+ (EXRL) Change", "Patients Per GP FTE (EXRL)"),
            filter = 'top',
            rownames = FALSE,
            options = list(dom = 'ltp'))
```
...
LKUP2%>% 
  ...
  datatable(colnames = c("Practice Code","Practice Name","PCN", "Practice Ranking","Risk","Risk Last Month", "Nurse Headcount","Nurse Headcount Last Month","Nurse Headcount Change", "Nurse FTE","Nurse FTE Last Month","Change in Nurse FTE Last Month", "Nurses aged 55+","Nurses aged 55+ Last Month","Nurses aged 55+ Change","Patients Per Nurse FTE"),
            filter = 'top',
            rownames = FALSE,
            options = list(dom = 'ltp',
...
Data updated to: `r format(Workforce_List_Date, format="%B %Y")`
...
LKUP2 %>% 
  left_join(`ContractEarnings`, by= c("MergePracticeCode" = "Practice Code"))%>% 
  dplyr::select(`MergePracticeCode`,`PracticeDescription`,PrimaryCareNetworkDescription, `Practice Total`) %>% 
  mutate(`Practice Total` = dollar_format(prefix="£")(`Practice Total`)) %>% 
  datatable(colnames = c("Practice Code","Practice Name","PCN","Total Contract Earnings"),
            filter = 'top',
            rownames = FALSE,
            options = list(dom = 'ltp'))
```
...
LKUP2%>% 
  left_join(MM_Diabeties, by= c("MergePracticeCode" = "...2")) %>% 
  datatable(colnames = c("Practice Code","Practice Name","PCN","Proportion"),
            filter = 'top',
            rownames = FALSE,
            options = list(dom = 'ltp',
                            columnDefs = list(list(className = 'dt-right', targets = 3:3))))
```
...
``` {r survey, echo = FALSE}
LKUP2 %>% 
  left_join((GPSurvey %>% dplyr::select(`Practice code`, 
                                        Score_getting_through_2021, Score_getting_through_2022, 
                                        Score_appt_experience_2021, Score_appt_experience_2022,
                                        Score_overall_experience_2021, Score_overall_experience_2022, RiskC)), 
         by= c("MergePracticeCode" = "Practice code")) %>%
  mutate(Score_getting_through_2021 = percent(Score_getting_through_2021,1),
         Score_getting_through_2022 = percent(Score_getting_through_2022,1),
         Score_appt_experience_2021 = percent(Score_appt_experience_2021,1), 
         Score_appt_experience_2022 = percent(Score_appt_experience_2022,1),
         Score_overall_experience_2021 = percent(Score_overall_experience_2021,1),
         Score_overall_experience_2022 = percent(Score_overall_experience_2022,1)) %>% 
  datatable(colnames = c("Practice Code","Practice Name","PCN",
...
The Nurse Risk score is used to rank the practices. If a practice is ranked between 1-16, we consider this to be a low risk. If a practice is ranked 17-48, we consider this to be a medium risk. If a practice is ranked 49-63, we consider this to be a high risk.
