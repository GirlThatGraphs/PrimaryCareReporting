...
#
GP_List <- read_excel("O:/BSS/BI/Intelligence Services/BSSW/Somerset/Reports/PrimaryCareReporting/Data/GP List.xlsx")
#
```
...
GP_Contract_Database_Contact_Lists_Live <-
  read_excel("//somerset.xswhealth.nhs.uk/ccg/Directorate/Clinical Commissioning Development/Comm & PC Services/PRIMARY CARE/Enhanced Services/GP/GP Contract Database & Contact Lists - Live.xlsx",
    skip = 3)
#
...
GP_Sizing_Tool <-
  read_excel(
    "O:/BSS/BI/Intelligence Services/BSSW/Somerset/Reports/PrimaryCareReporting/Data/NEW - GP Sizing Tool (care WIP 2016 tool with updated list sizes).xls",
    sheet = "Premises Data",
    range = "A4:AH88")

GP_Sizing_ListFilter <- GP_Sizing_Tool %>%
...
#
MM_Green <- read_excel("Data/MM scorecard 2022-23 September final.xlsx",
                       sheet = "Ranked scorecard 2022-23",
                       range = "A1:AX65")
#
MM_Date <- "September 2022"
#
MM_Green <- MM_Green %>%
            filter(`...2` != 'L85612') %>% 
...
MMGreenCount <- MM_Green %>%
                ...
                pull()
#
MMDiabeties <- MM_Green %>%
                filter(...2 == code) %>%
                select(`11. Patients with all 8 diabetes care processes undertaken in the last 12 months`) %>%
                pull()
```
...
GPSurveya <- read_excel("O:/BSS/BI/Intelligence Services/BSSW/Somerset/Reports//PrimaryCareReporting/Data/GPSurvey.xlsx") %>% 
...
#
WorkforceAll <- read_excel("O:/BSS/BI/Intelligence Services/BSSW/Somerset/Reports//PrimaryCareReporting/Data/Workforce2.xlsx") %>%
  drop_na(MONTH_DATE) |>   
...
SOMRatio <- WorkforceAll |> 
  filter(MONTH_DATE == max(MONTH_DATE)) |> 
  mutate(`TOTAL_GP_FTE_REGULAR` = round(TOTAL_GP_EXTGL_FTE,1),
  ...
  summarise_all(sum) |> 
  mutate(Patient_GP_ratio = round(TOTAL_PATIENTS / TOTAL_GP_FTE_REGULAR),
        Patient_NURSES_ratio = round(TOTAL_PATIENTS / TOTAL_NURSES_FTE))

SOMRatio_GP <- SOMRatio |> 
  select(Patient_GP_ratio) |> 
  pull()

SOMRatio_Nurse <- SOMRatio |> 
  select(Patient_NURSES_ratio) |> 
  pull()
...
WorkforceList <- WorkforceAll %>%
  select(`MONTH_DATE`,
         `MergePracticeCode`,
         `TOTAL_PATIENTS`) %>%
  filter(month(MONTH_DATE) %in% c(3,6,9,12)) |>  
  group_by(`MONTH_DATE`,
           `MergePracticeCode`) %>%
  summarise_all(sum, na.rm = T) %>%
  ungroup() %>% 
  ...
  mutate(CHANGE_PATIENTS = TOTAL_PATIENTS - lag(TOTAL_PATIENTS),
         CHANGE_PATIENTS_ratio = (TOTAL_PATIENTS / lag(TOTAL_PATIENTS)-1)) %>% 
  ungroup() %>% 
  mutate(Risk_List_Change =  case_when(CHANGE_PATIENTS_ratio >= 0.02 ~ 12,
                                       CHANGE_PATIENTS_ratio <= -0.02 ~ 12,
                                       CHANGE_PATIENTS_ratio >= 0.01 ~ 8,
                                       CHANGE_PATIENTS_ratio <= -0.01 ~ 8,
                                       TRUE ~ 0)) %>%
  mutate(`List_Risk` = Risk_List_Change) %>% 
  group_by(MONTH_DATE) %>% 
  mutate(`List_Risk_Rank` = rank(`List_Risk`)) %>%
  mutate(List_Risk_RankC = case_when(Risk_List_Change == 0 ~ "low",
                                     Risk_List_Change == 12 ~ "high",
                                     Risk_List_Change == 8 ~ "medium",
                                     TRUE ~ "error"),
...
```{r ContractEarnings, include = FALSE}
`ContractEarnings` <- read_excel("Data/Practice Contract Earnings 2021-22 - All CCG Spend incl. Prescribing V3.xlsx", 
    skip = 6)
...
LD_Health_CheckIn <- read_excel("Data/LD health Check Scheme Practice Payment 22-23.xlsx",
                                range = "A3:AO67")

LD_Health_Check<- LD_Health_CheckIn %>%
  mutate(Date = case_when (is.na(`CRP...6`)~"M1",
                           is.na(`CRP...9`)~"M2",
                           is.na(`CRP...12`)~"M3",
                           is.na(`CRP...15`)~"M4",
                           is.na(`CRP...18`)~"M5",
...
LD_Perf<-LD_Health_Check %>% 
  select(`Total percentgage of health checks undertaken in 2022/23\r\n(based on average registered size)`) %>% 
  pull()
```
...
#
REFERENCE_CQC <- tbl(SQL_CONNECTION, in_schema("SOM","REFERENCE_CQC5"))
...
NationalQOF <- read_excel("Data/QOF_National_2122.xlsx") 
```
...
QOFCCG2223 <- QOF_2223 %>%
  ...
  summarise(`NumeratorT` = sum(`Numerator`),
            `DenominatorT` = sum(`Denominator`)) %>% 
  ungroup() %>%
  mutate(`AchievementT2223` = (`NumeratorT` / `DenominatorT`))
...
QOF <- QOF_2122 %>%
  ...
  left_join(QOFCCG2223, by=c("IND_CODE"="IND_CODE")) %>% 
  group_by(IND_CODE) %>% 
  ...
  mutate(`2223Achievement`=scales::percent(`2223Achievement`, accuracy = 1),
         ...
         `2223SomAchievement`=scales::percent(AchievementT2223, accuracy = 1)) %>% 
  filter(ORG_CODE==code) %>% 
  left_join(NationalQOF, by = c("IND_CODE"="IND_CODE") ) %>% 
  mutate(`2122 National Achievement`=scales::percent(`NAT2122`/100, accuracy = 1))
```
...
web_data2 <- web_data |> 
  data.frame() |> 
  filter(grepl("ICS",web_data) | grepl("%",web_data))
```
...
TInd1a <- WorkforceGP %>% 
         ...
         mutate(Indicator = "GP Workforce")
  
TInd1b <- WorkforceNURSE %>% 
         ...
         mutate(Indicator = "Nurse Workforce")
  
TInd1c <- WorkforceList %>% 
...
TInd3 <- MM_Green %>% 
        ...
        mutate(Green_RankC = cell_spec(Green_RankC,"latex", bold = T, 
         ...
         mutate(Indicator = "Medicines Management Scorecard") 
...
TInd4 <- GPStandard %>% 
        ...
        mutate(`RiskC` = revalue(`RiskC`, c("high"="h", "medium"="m", "low"="l"))) %>% 
        mutate(RiskC = cell_spec(RiskC,"latex", bold = T, color = ifelse(RiskC == "h", "red", ifelse(RiskC == "l", "green", "orange"))))   |> 
         mutate(Indicator = "A and E Attendances") 
...
TInd5 <- GPStandard %>% 
        ...
        mutate(`RiskC` = revalue(`RiskC`, c("high"="h", "medium"="m", "low"="l"))) %>% 
        mutate(RiskC = cell_spec(RiskC,"latex", bold = T, color = ifelse(RiskC == "h", "red", ifelse(RiskC == "l", "green", "orange"))))    |> 
         mutate(Indicator = "Emergency Admissions") 

TInd11 <- AdviceGuidance %>% 
         ...
         mutate(Indicator = "Advice and Guidance") 
...
TInd14 <- GP_Database %>% 
         ...
         mutate(Risk = cell_spec(Risk,"latex", bold = T, color = ifelse(Risk == "h", "red", ifelse(Risk == "l", "green", "orange"))))    |> 
         mutate(Indicator = "Standard Contract Enhanced Services") 

names(TInd1b) <- names(TInd1a)
names(TInd1c) <- names(TInd1a)
names(TInd7) <- names(TInd1a)
names(TInd3) <- names(TInd1a)
names(TInd4) <- names(TInd1a)
names(TInd5) <- names(TInd1a)
names(TInd11) <- names(TInd1a)
names(TInd12) <- names(TInd1a)
names(TInd13) <- names(TInd1a)
names(TInd14) <- names(TInd1a)

IndicatorsTest <- rbind(TInd1a,
                        TInd1b,
                        TInd1c,
                        TInd7,
                        TInd3,
...
kable(
  REFERENCE_TEAM %>% 
    filter(Name %in% c("Daniel Jayeola", "Joshua Herron")),
  format = "latex",
  booktabs = T,
  align = "l",
  linesep = ""
...
Practices are given a ranking out of the 63 practices together with the
risk indicator with 1 considered the lowest risk and 63 considered the
...
```{r Indicator Table Test, echo = FALSE}
IndicatorsTest%>% filter(MergePracticeCode==code) %>% 
select(3,2) %>% 
kable(col.names = c("Indicator", "Risk Score"),
      format = "latex",
      escape = F,
      align = "l",
      booktabs = T,
...
The practice is ranked `r GP_Risk_Rank` of the 63 practices, we consider
this to be a
...
are aged 55 plus. This gives a GP to Patient ratio of
1:`r comma(round(TOTAL_PATIENTS/TOTAL_GP_FTE_REGULAR))`, compared to a Somerset average of 1:`r comma(SOMRatio_GP)`. The practice
has `r TOTAL_GP_PTNR_HC` GP Partners.

### Nurse & DCP

The practice is ranked `r Nurse_Risk_Rank` of the 63 practices, we
...
are aged 55 plus. This gives a Nurse to Patient ratio of
1:`r comma(round(TOTAL_PATIENTS/TOTAL_NURSES_FTE))`, compared to a Somerset average of 1:`r comma(SOMRatio_Nurse)`.
...
The premises risk is based on the Gross Holding Area compared to the Desired Area. The practice premises is `r scales::percent(Undersized)` undersized. This gives the practice an ICB ranking of `r UndersizedRank` out of 63 practices, and is considered a `r if (UndersizedRisk == "high") "\\textcolor{red}{high}" else if (UndersizedRisk == "medium") "\\textcolor{orange}{medium}" else "\\textcolor{green}{low}"` risk.
...
The practice is ranked `r List_Risk_Rank` out of 63 practices. We
consider this to be
...
finalise_plot(
  ...
  save_filepath = "O:/BSS/BI/Intelligence Services/BSSW/Somerset/Reports/PrimaryCareReporting/Plots/Survey_Chart.png") 
```
...
-   `r web_data2[1,]`
    ...
    `r gsub("(%)(N)", "\\1 \\2", web_data2[2,])`

-   `r web_data2[3,]`

    `r gsub("(%)(N)", "\\1 \\2", web_data2[4,])`

-   `r web_data2[5,]`

    `r gsub("(%)(N)", "\\1 \\2", web_data2[6,])`

### Where patient experience could improve

-   `r web_data2[7,]`

    `r gsub("(%)(N)", "\\1 \\2", web_data2[8,])`

-   `r web_data2[9,]`

    `r gsub("(%)(N)", "\\1 \\2", web_data2[10,])`

-   `r web_data2[11,]`

    `r gsub("(%)(N)", "\\1 \\2", web_data2[12,])`
...
the practice an ICB ranking of `r MMRank` out of 63 practices, which is
considered a
...
In the 12 months to `r MM_Date`, the proportion of patients with all eight care processes is `r percent(MMDiabeties)`.
The target is to achieve at least 70%.
...
The table below gives current in year 2022/23 practice and Somerset achievement within the ICBs key clinical priority areas as at `r as.Date(as.character(QOFDate), "%Y%m%d")`. Also shown are the 2022/23 achievements at practice, Somerset and National level.
...
QOF %>%
  ...
  tab_spanner(label = "2122 Achievement",
              columns = vars(`2122Achievement`,
                             `2122SomAchievement`,
                             `2122 National Achievement`)) %>%
    tab_spanner(label = "2223 Achievement",
  ...
  cols_label(`2223Achievement` = ("Practice"),
             `2122Achievement` = ("Practice"),
             `2122SomAchievement` = ("Somerset"),
             `2223SomAchievement` = ("Somerset"),
             `2122 National Achievement` = ("National"))
```

For the 2022/23 QOF Guidance, please see: \href{https://www.england.nhs.uk/wp-content/uploads/2022/03/B133-update-on-quality-outcomes-framework-changes-for-22-23-guidance.pdf}{{\color{blue}{\underline{here}}}}.
...
At `r LD_Health_CheckDate` 2022/23 the practice has currently
...
The practice is ranked `r AERank` of the 63 practices. We consider this
to be a
...
finalise_plot(
  ...
  save_filepath = "O:/BSS/BI/Intelligence Services/BSSW/Somerset/Reports/PrimaryCareReporting/Plots/SUS_STANDARDISATION_ED_PLOT.png")
```
...
activity than we would expect compared to the standard Somerset model.
The practice is ranked `r NELRank` of the 63 practices. We consider this
to be a
...
finalise_plot(
  SUS_STANDARDISATION_EM_PLOT,
  source = "Data sourced via SUS",
  save_filepath = "O:/BSS/BI/Intelligence Services/BSSW/Somerset/Reports/PrimaryCareReporting/Plots/SUS_STANDARDISATION_EM_PLOT.png")
```
...
finalise_plot(
  SUS_STANDARDISATION_EM_Cardio_PLOT,
  source = "Data sourced via SUS",
  save_filepath = "O:/BSS/BI/Intelligence Services/BSSW/Somerset/Reports/PrimaryCareReporting/Plots/SUS_STANDARDISATION_EM_Cardio_PLOT.png")
```
...
finalise_plot(
  SUS_STANDARDISATION_EM_Diab_PLOT,
  source = "Data sourced via SUS",
  save_filepath = "O:/BSS/BI/Intelligence Services/BSSW/Somerset/Reports/PrimaryCareReporting/Plots/SUS_STANDARDISATION_EM_Diab_PLOT.png")
```
...
finalise_plot(
  SUS_STANDARDISATION_EM_Resp_PLOT,
  source = "Data sourced via SUS",
  save_filepath = "O:/BSS/BI/Intelligence Services/BSSW/Somerset/Reports/PrimaryCareReporting/Plots/SUS_STANDARDISATION_EM_Resp_PLOT.png")
```
...
finalise_plot(
  SUS_STANDARDISATION_EM_Neoplasms_PLOT,
  source = "Data sourced via SUS",
  save_filepath = "O:/BSS/BI/Intelligence Services/BSSW/Somerset/Reports/PrimaryCareReporting/Plots/SUS_STANDARDISATION_EM_Neoplasms_PLOT.png")
```
...
finalise_plot(
  SUS_STANDARDISATION_OP_PLOT,
  source = "Data sourced via SUS",
  save_filepath = "O:/BSS/BI/Intelligence Services/BSSW/Somerset/Reports/PrimaryCareReporting/Plots/SUS_STANDARDISATION_OP_PLOT.png")
```
...
`r AGRank` of the 63 practices. We consider this to be a
...
finalise_plot(
  ...
  save_filepath = "O:/BSS/BI/Intelligence Services/BSSW/Somerset/Reports/PrimaryCareReporting/Plots/AG_PLOT.png")
```
...
The GP Risk score is used to rank the practices. If a practice is ranked between 1-16, we consider this to be a low risk. If a practice is ranked 17-48, we consider this to be this is a medium risk. If a practice is ranked 49-63, we consider this to be a high risk.
...
ranked 49-63, we consider this to be a high risk.

### Patient List

The patient list risk score depends on the quarterly change in list size:

If the list has changed by less than 1% then we consider this to be a low risk. 
If the list has increased or decreased between 1%-2% then we consider this to be a medium risk. 
If the list has increased or decreased by more than 2% we consider this to be a high risk.
...
The core DESs are considered to be Network Contract & Learning Disabilities Health Check Scheme.

Practices providing both core DES are considered low risk. Those
providing a 0 or 1 of the core DES are considered high risk.
...
to be a medium risk. If the practice is ranked between 49-63 we consider
...
49-63 we consider this to be a high risk.
