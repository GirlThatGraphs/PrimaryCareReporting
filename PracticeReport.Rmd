...
QOFCCG1920 <- QOF_1920 %>%
  ...
  ungroup() %>%
  mutate(`AchievementT` = (`NumeratorT` / `DenominatorT`))
#
QOF_1920 <- QOF_1920 %>%
  filter(`FIELD_NAME` %in% c("Denominator", "Numerator")) %>%
  collect() %>%
...
QOF <- QOF_2122 %>%
  ...
  mutate(`2223Achievement`=scales::percent(`2223Achievement`, accuracy = 1),
         ...
         `1920SomAchievement`=scales::percent(AchievementT, accuracy = 1)) %>% 
  filter(ORG_CODE==code) %>% 
  left_join(X1920QOF, by = c("IND_CODE"="IND_CODE") ) %>% 
  mutate(`1920 National Achievement`=scales::percent(`NAT`/100, accuracy = 1))
```
